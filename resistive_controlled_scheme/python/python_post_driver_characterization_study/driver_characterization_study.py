...
full_data = np.zeros( [initial_gaps.shape[0], simulated_levels] )
for g_idx, g in enumerate(initial_gaps):
    data_file = cadence_folder + 'driver_characterization_' + cell + '_g_' + str(g_idx) +'.csv'
    # data is in two columns
    # (level, read_data)
    full_data[g_idx] = np.genfromtxt(data_file, delimiter=',')[1:, 1]
...
if export_ocean:
    ...
    full_data[g_idx] = np.genfromtxt(data_file, delimiter=',')
...
if clip_r_read:
    for g_idx, g in enumerate(last_r_read):
        ...
        print('\t* gap #' + str(g_idx))
        print('\t* g_' + str(g_idx) + ' read range: ' + str(g[min_range_idx[g_idx]]) + '-' + str(g[max_range_idx[g_idx]]) )
        print('\t* g_' + str(g_idx) + ' load range: ' + str(r_loads[min_range_idx[g_idx]]) + '-' + str(r_loads[max_range_idx[g_idx]]) )
...
for g_idx, g in enumerate(last_r_read):
    sim_read_r[g_idx] = np.linspace(g[min_range_idx[g_idx]],
                                    g[max_range_idx[g_idx]], target_levels)
...
for g_idx, g in enumerate(sim_read_r):
    for t_idx, r in enumerate(g):
        ...
        # the search finds between min_range_idx[g_idx]:max_range_idx[g_idx]]
        # we use the absolute index r_idx + min_range_idx[g_idx]
